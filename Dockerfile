...
ARG filename=polyglot.linux.x86_64.pyz
ARG binfile=$dir/$filename
ARG user=polyglot
ARG group=$user
ARG uid=1000
ARG gid=1000
...
RUN wget https://github.com/UniversalDevicesInc/Polyglot/raw/unstable-release/bin/${filename} --output-document=${binfile}
